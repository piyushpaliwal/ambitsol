...
var engagement = window.location.search.substring(12);

if( engagement != '' ){
	var val = document.getElementById('template-contactform-service');
	val.selectedIndex = engagement;
}
...
var contactForm = document.getElementById('template-contactform');

contactForm.addEventListener('submit', (e)=>{
	// e.preventDefault();
	// var formData = new FormData(document.getElementById('template-contactform'));
	var name = contactForm.elements["template-contactform-name"].value;
	var email = contactForm.elements["template-contactform-email"].value;
	var phone = contactForm.elements["template-contactform-phone"].value;
	var subject = contactForm.elements["template-contactform-subject"].value;
	var model = contactForm.elements["template-contactform-service"].value;
	var message = contactForm.elements["template-contactform-message"].value;
	
	if(name != '' && email != '' && model != '' && message != ''){
		var xhr = new XMLHttpRequest();
		var url = "https://prod-13.centralindia.logic.azure.com:443/workflows/8f690036d23647688f0cbf6f14701cf9/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=ad4wWhdr3kmhYzum4fvSe7jzFVPS4OEYvk9-g696SPI";
		...
		var elementResult = document.querySelector('.contact-form-result');
		elementResult.classList.add('alert', 'alert-success');
		elementResult.style.display = 'none';
		elementResult.setAttribute('data-notify-type', 'success');
		elementResult.setAttribute('data-notify-msg', 'Message Sent Successfully!')
		//elementResult.attr( 'data-notify-type', 'success' ).attr( 'data-notify-msg', 'Message Sent Successfully!' ).html('');
		SEMICOLON.widget.notifications( elementResult );

		var data = JSON.stringify({"email": email, "regarding": model, "name": name, "message": message});
		xhr.send(data);
		$('form').clearForm();
	}
